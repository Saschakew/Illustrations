...
function initializeApp() {
  ...
  if (document.body.className === 'page6') {
    ...
    updateNonGaussianityDisplay();
     ...
     // Typeset MathJax content (guarded)
...
function setupEventListeners() {
  ...
  function createPopup(icon, className) {
    ...
    // Process LaTeX in the popup (guarded)
...
function updateAllMatrices(phi0, phi) {
  ...
  // Trigger MathJax to render the new content (targeted and guarded)
  if (typeof MathJax !== 'undefined' && MathJax.typesetPromise) {
    const targets = [b0Element, bElement].filter(el => el && el.isConnected);
    if (targets.length) {
      MathJax.typesetPromise(targets).catch((err) => console.warn('MathJax typeset error:', err));
    }
  }
...
function updateNonGaussianityDisplay() {
  ...
  if (displayElement) {
    ...
    // Trigger MathJax to render the new content (guarded)
    if (typeof MathJax !== 'undefined' && MathJax.typesetPromise) {
      MathJax.typesetPromise([displayElement]).catch((err) => console.warn('MathJax typeset error:', err));
    }
  }
}
